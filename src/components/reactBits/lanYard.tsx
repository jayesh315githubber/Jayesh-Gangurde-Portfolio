...
const profilePhoto = "/Jayesh_Gangurde.webp";

extend({ MeshLineGeometry, MeshLineMaterial });

// Function to create premium navy blue ribbon texture
function createPremiumRibbonTexture(
  width: number = 1024,
  height: number = 1024
): THREE.CanvasTexture {
  ...
  ctx.save();
  ctx.globalAlpha = 0.2;
  const radialDepth = ctx.createRadialGradient(
    width / 2,
    height / 2,
    0,
    width / 2,
  ...
  ctx.save();
  ctx.globalAlpha = 0.4;
  const centerStripe = ctx.createLinearGradient(
    width / 2 - width * 0.15,
    0,
    width / 2 + width * 0.15,
    0
  ...
  const secondaryStripe = ctx.createLinearGradient(
    width / 2 - width * 0.08,
    0,
    width / 2 + width * 0.08,
    0
...
function createIDCardTexture(
  ...
  ctx.arc(
    width / 2,
    punchHoleY - punchHoleRadius * 0.2,
    punchHoleRadius * 0.3,
    0,
    Math.PI * 2
  );
  ...
  ctx.font = `900 ${
    width * 0.095
  }px -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Inter', 'SF Pro Display', Roboto, sans-serif`;
  ...
  ctx.fillText("Jayesh Gangurde", width / 2, nameY);
  ctx.shadowBlur = 0;
...
function Band({
  ...
  const [ribbonTexture, setRibbonTexture] =
  ...
  const [idCardTexture, setIdCardTexture] = useState<THREE.CanvasTexture>(
    () => {
      ...
      placeholderTexture.wrapS = placeholderTexture.wrapT =
  ...
  useEffect(() => {
    if (
      ...
      if (
        photoTexture.image instanceof HTMLImageElement &&
        photoTexture.image.complete
      ) {
        ...
        photoTexture.image.onload = () => {
          if (!textureUpdateRef.current) {
            ...
            textureUpdateRef.current = true;
            const cardTexture = createIDCardTexture(
              null,
              photoTexture.image as HTMLImageElement
            );
            cardTexture.wrapS = cardTexture.wrapT = THREE.ClampToEdgeWrapping;
            cardTexture.flipY = true;
            setIdCardTexture(cardTexture);
          ...
          <group
            ...
            onPointerUp={(e: any) => {
              ...
              if (
                ...
                nativeEvent.pointerId === pointerIdRef.current
              ) {
                try {
                  if (
                    nativeEvent.target &&
                    typeof nativeEvent.target.releasePointerCapture ===
                      "function"
                  ) {
            ...
            onPointerDown={(e: any) => {
              ...
              try {
                if (
                  nativeEvent.target &&
                  typeof nativeEvent.target.setPointerCapture === "function"
                ) {
