    ...
    () => {
      ...
      mm.add(
        ...
        () => {
          ...
          if (subtitleRef.current) {
            gsap.set(subtitleRef.current, { opacity: 0, filter: "blur(6px)", y: 10 });
            tl.to(subtitleRef.current, {
              opacity: 1,
              filter: "blur(0px)",
              y: 0,
      ...
      mm.add("(prefers-reduced-motion: reduce)", () => {
        [iconRef.current, titleRef.current, subtitleRef.current].forEach((el) => {
          if (el) gsap.set(el, { opacity: 1, clearProps: "transform,filter,clipPath" });
        });
      });
    },
    { scope: pageRef }
    ...
    <div ref={pageRef} className="min-h-screen bg-transparent relative route-enter-content">
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-8 md:py-12">
        ...
        <div className="mb-8 flex items-center justify-between flex-wrap gap-3 sm:gap-4 route-enter-child">
          <button
            onClick={(e) => {
              ...
              navigate("/", {
                state: { scrollTo: "hero", scrollY, from: "home-to-project" },
              });
            }}
          ...
          <ShareButton
            shareData={{
              title: "Site Architecture - Jayesh Gangurde's Portfolio",
              description: "Interactive site architecture visualizer with drag-and-drop reordering and GSAP animations.",
              url: getCurrentUrl(),
            }}
            variant="outline"
            size="md"
        ...
        <div className="mb-10 md:mb-14 text-center route-enter-child">
          ...
          >
            Interactive visualization of the portfolio's page hierarchy.
            Drag sections to reorder, click nodes to navigate.
          </p>
        </div>
